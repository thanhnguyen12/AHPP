...
**Note:** we will use 'None' = 1 for all categories as *None* signifies that the house does not have that particular house and regardless of the house quality or sale price will be set to 0. 
...
```{r warning=FALSE, message=FALSE}
bst = xgb.train(xgb_params,dtrain, nrounds = 2000)#, early_stopping_rounds = 400, watchlist = list(train=dtrain))
```

**Running XGBoost in a Kaggle Kernel is too computationally heavy and takes too long, however, depending on the $\eta$ I would increase `nrounds` to somewhere around 10000.
This all really depends on how you tune your parameters with `expand.grid`.**




**Running XGBoost in a Kaggle Kernel is too computationally heavy and takes too long, however, running on your desktop should take no time at all. Uncomment these for stronger predicting**
